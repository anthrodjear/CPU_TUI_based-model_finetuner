from typing import Optional, Dict, Any, List
from pathlib import Path
import time
import yaml

from utils.logger import LoggerMixin, load_config
from utils.system_monitor import SystemMonitor
from models.model_scanner import ModelScanner
from models.ollama_bridge import OllamaBridge
from dataset.dataset_loader import DatasetLoader
from dataset.dataset_analyzer import DatasetAnalyzer
from training.trainer import Trainer
from training.lora_config import LoRAConfig
from export.adapter_exporter import AdapterExporter
from export.gguf_converter import GGUFConverter
from visualization.dashboard import Dashboard


class ResourceManager(LoggerMixin):
    """Manage system resources and validate training parameters."""
    
    def __init__(self, config: Optional[dict] = None):
        self.config = config or load_config()
        self.system_monitor = SystemMonitor()
    
    def validate_training_params(
        self,
        model_size_gb: float,
        batch_size: int,
        seq_length: int,
        lora_r: int,
        preset: Optional[str] = None
    ) -> Dict[str, Any]:
        """Validate training parameters against available resources."""
        if preset:
            preset_config = self.config.get('resource_presets', {}).get(preset, {})
            batch_size = preset_config.get('batch_size', batch_size)
            seq_length = preset_config.get('max_seq_length', seq_length)
            lora_r = preset_config.get('lora_r', lora_r)
        
        memory_estimate = self.system_monitor.estimate_memory_usage(
            model_size_gb=model_size_gb,
            batch_size=batch_size,
            seq_length=seq_length,
            lora_r=lora_r
        )
        
        total_estimate = memory_estimate['total_estimate_gb']
        available_memory = self.system_monitor.get_available_memory_gb()
        
        safety_margin = self.config.get('memory', {}).get('safety_margin', 0.8)
        adjusted_available = available_memory * safety_margin
        
        warnings = []
        errors = []
        
        if total_estimate > adjusted_available:
            errors.append(
                f"Insufficient memory: need {total_estimate:.1f}GB, "
                f"available ~{adjusted_available:.1f}GB"
            )
        
        if total_estimate > available_memory * 0.95:
            warnings.append("Critical memory usage - training may crash")
        
        suggested_batch = batch_size
        suggested_seq = seq_length
        suggested_lora_r = lora_r
        
        if errors:
            if batch_size > 1:
                suggested_batch = 1
                warnings.append("Reduced batch size to 1")
            
            if seq_length > 512:
                suggested_seq = 512
                warnings.append("Reduced sequence length to 512")
            
            if lora_r > 8:
                suggested_lora_r = 8
                warnings.append("Reduced LoRA rank to 8")
        
        return {
            "valid": len(errors) == 0,
            "errors": errors,
            "warnings": warnings,
            "memory_estimate": memory_estimate,
            "available_memory": available_memory,
            "suggested_params": {
                "batch_size": suggested_batch,
                "max_seq_length": suggested_seq,
                "lora_r": suggested_lora_r
            }
        }
    
    def get_optimal_params(
        self,
        model_size_gb: float,
        preset: str = "balanced"
    ) -> Dict[str, int]:
        """Get optimal training parameters based on resources."""
        preset_config = self.config.get('resource_presets', {}).get(preset, {})
        
        available_memory = self.system_monitor.get_available_memory_gb()
        
        if available_memory >= 16:
            return {
                "batch_size": preset_config.get('batch_size', 2),
                "max_seq_length": preset_config.get('max_seq_length', 768),
                "lora_r": preset_config.get('lora_r', 16),
                "gradient_accumulation": preset_config.get('gradient_accumulation', 2)
            }
        elif available_memory >= 12:
            return {
                "batch_size": 2,
                "max_seq_length": 512,
                "lora_r": 8,
                "gradient_accumulation": 4
            }
        else:
            return {
                "batch_size": 1,
                "max_seq_length": 512,
                "lora_r": 4,
                "gradient_accumulation": 8
            }
    
    def check_disk_space(self, required_gb: float) -> bool:
        """Check if sufficient disk space is available."""
        import shutil
        
        try:
            path = Path(self.config.get('system', {}).get('checkpoint_dir', './checkpoints'))
            total, used, free = shutil.disk_usage(path if path.exists() else Path.cwd())
            
            free_gb = free / (1024 ** 3)
            return free_gb >= required_gb
        except Exception:
            return True


class JobManager(LoggerMixin):
    """Manage fine-tuning jobs."""
    
    def __init__(self, config: Optional[dict] = None):
        self.config = config or load_config()
        self.resource_manager = ResourceManager(config)
        self.current_job: Optional[Dict[str, Any]] = None
    
    def create_job(
        self,
        model_path: str,
        dataset_path: str,
        job_name: str,
        num_epochs: int = 3,
        batch_size: int = 2,
        max_seq_length: int = 512,
        learning_rate: float = 3e-4,
        lora_r: int = 8,
        lora_alpha: int = 16,
        preset: Optional[str] = None
    ) -> Dict[str, Any]:
        """Create a new fine-tuning job."""
        model_scanner = ModelScanner(self.config)
        
        from models.model_info import ModelInfo
        model_info = ModelInfo(
            name=Path(model_path).name,
            path=model_path,
            size_gb=1.0,
            quantization=None,
            architecture=None,
            context_length=4096,
            file_type="bin"
        )
        
        validation = self.resource_manager.validate_training_params(
            model_size_gb=model_info.size_gb,
            batch_size=batch_size,
            seq_length=max_seq_length,
            lora_r=lora_r,
            preset=preset
        )
        
        if not validation["valid"]:
            raise ValueError(f"Invalid parameters: {validation['errors']}")
        
        output_dir = Path(self.config.get('system', {}).get('output_dir', './output'))
        job_output = output_dir / job_name
        job_output.mkdir(parents=True, exist_ok=True)
        
        self.current_job = {
            "name": job_name,
            "model_path": model_path,
            "dataset_path": dataset_path,
            "output_dir": str(job_output),
            "num_epochs": num_epochs,
            "batch_size": validation["suggested_params"]["batch_size"],
            "max_seq_length": validation["suggested_params"]["max_seq_length"],
            "learning_rate": learning_rate,
            "lora_r": validation["suggested_params"]["lora_r"],
            "lora_alpha": lora_alpha,
            "status": "created",
            "created_at": time.time()
        }
        
        return self.current_job
    
    def get_job_status(self) -> Dict[str, Any]:
        """Get current job status."""
        return self.current_job or {"status": "no_job"}
    
    def update_job_status(self, status: str, metrics: Optional[Dict] = None):
        """Update job status."""
        if self.current_job:
            self.current_job["status"] = status
            self.current_job["updated_at"] = time.time()
            if metrics:
                self.current_job["metrics"] = metrics


class Orchestrator(LoggerMixin):
    """Main orchestrator for the fine-tuning studio."""
    
    def __init__(self, config: Optional[dict] = None):
        self.config = config or load_config()
        
        self.model_scanner = ModelScanner(self.config)
        self.ollama_bridge = OllamaBridge(self.config)
        self.dataset_loader = DatasetLoader(self.config)
        self.dataset_analyzer = DatasetAnalyzer(self.config)
        self.resource_manager = ResourceManager(self.config)
        self.job_manager = JobManager(self.config)
        
        self.trainer: Optional[Trainer] = None
        self.dashboard: Optional[Dashboard] = None
    
    def scan_models(self) -> List:
        """Scan for available Ollama models."""
        return self.model_scanner.scan_models()
    
    def analyze_dataset(
        self,
        dataset_path: str,
        tokenizer=None
    ) -> Dict[str, Any]:
        """Analyze a dataset."""
        samples = self.dataset_loader.load(dataset_path)
        
        return self.dataset_analyzer.analyze(samples, tokenizer)
    
    def create_job(
        self,
        model_path: str,
        dataset_path: str,
        job_name: str,
        **kwargs
    ) -> Dict[str, Any]:
        """Create a new fine-tuning job."""
        return self.job_manager.create_job(
            model_path=model_path,
            dataset_path=dataset_path,
            job_name=job_name,
            **kwargs
        )
    
    def run_training(
        self,
        job_config: Dict[str, Any],
        resume_from_checkpoint: Optional[str] = None
    ):
        """Run training for a job."""
        self.trainer = Trainer(self.config, self.resource_manager.system_monitor)
        
        lora_config = LoRAConfig(
            r=job_config.get('lora_r', 8),
            lora_alpha=job_config.get('lora_alpha', 16)
        )
        
        self.dashboard = Dashboard(self.config)
        self.dashboard.initialize(job_config['output_dir'])
        
        self.job_manager.update_job_status("training")
        
        try:
            self.trainer.train(
                model_path=job_config['model_path'],
                dataset_path=job_config['dataset_path'],
                output_dir=job_config['output_dir'],
                num_epochs=job_config.get('num_epochs', 3),
                batch_size=job_config.get('batch_size', 2),
                max_length=job_config.get('max_seq_length', 512),
                learning_rate=job_config.get('learning_rate', 3e-4),
                lora_config=lora_config,
                resume_from_checkpoint=resume_from_checkpoint
            )
            
            self.job_manager.update_job_status("completed")
            
            return True
        
        except Exception as e:
            self.logger.error(f"Training failed: {e}")
            self.job_manager.update_job_status("failed", {"error": str(e)})
            raise
    
    def export_adapter(
        self,
        job_config: Dict[str, Any],
        output_path: str,
        merge_base: bool = False
    ) -> str:
        """Export the trained adapter."""
        if not self.trainer:
            raise ValueError("No trained model available")
        
        exporter = AdapterExporter(self.config)
        
        if merge_base:
            return exporter.export_merged_model(
                self.trainer.model,
                self.trainer.tokenizer,
                output_path,
                job_config['model_path']
            )
        else:
            return exporter.export_adapter(
                self.trainer.model,
                self.trainer.tokenizer,
                output_path
            )
    
    def install_in_ollama(
        self,
        model_path: str,
        model_name: str,
        temperature: float = 0.7
    ) -> bool:
        """Install a model in Ollama."""
        modelfile_path = Path(model_path) / "Modelfile"
        
        self.ollama_bridge.generate_modelfile(
            base_model=model_path,
            temperature=temperature,
            output_path=str(modelfile_path)
        )
        
        return self.ollama_bridge.create_model(model_name, str(modelfile_path))
    
    def generate_report(self, output_dir: str):
        """Generate training report."""
        if self.trainer and self.trainer.metrics_tracker:
            summary = self.trainer.metrics_tracker.get_summary()
            
            if self.dashboard:
                self.dashboard.generate_report(summary)
                self.dashboard.save_training_plot(
                    self.trainer.metrics_tracker.get_graph_data("loss")
                )
            
            return summary
        
        return {}
